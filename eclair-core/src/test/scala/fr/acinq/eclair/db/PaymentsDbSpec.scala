...
import fr.acinq.bitcoinscala.{Block, ByteVector32, Crypto}
...
class PaymentsDbSpec extends AnyFunSuite {
  ...
  test("migrate sqlite payments db v1 -> current") {
    ...
    migrationCheck(
      ...
      postCheck = _ => {
        ...
        val pr1 = IncomingPayment(i1, preimage1, PaymentType.Standard, i1.createdAt.toTimestampMilli, IncomingPaymentStatus.Received(550 msat, 1100 unixms))
  ...
  test("migrate sqlite payments db v2 -> current") {
    ...
    val pr1 = IncomingPayment(i1, preimage1, PaymentType.Standard, i1.createdAt.toTimestampMilli, IncomingPaymentStatus.Received(12345678 msat, 1090 unixms))
    ...
    val pr2 = IncomingPayment(i2, preimage2, PaymentType.Standard, i2.createdAt.toTimestampMilli, IncomingPaymentStatus.Expired)
  ...
  test("migrate postgres payments db v4 -> current") {
    ...
    val pr1 = IncomingPayment(i1, preimage1, PaymentType.Standard, i1.createdAt.toTimestampMilli, IncomingPaymentStatus.Received(12345678 msat, TimestampMilli.now()))
  ...
  test("high level payments overview") {
    ...
    val receivedPayment2 = IncomingPayment(paidInvoice2, randomBytes32(), PaymentType.Standard, paidInvoice2.createdAt.toTimestampMilli, IncomingPaymentStatus.Received(889 msat, receivedAt2))
