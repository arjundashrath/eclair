...
import fr.acinq.bitcoinscala.Block
...
class PaymentInitiatorSpec extends TestKitBaseClass with FixtureAnyFunSuiteLike {
  ...
  test("forward payment with user custom tlv records") { f =>
    ...
    val req = SendPaymentToNode(finalAmount, invoice, 1, Channel.MIN_CLTV_EXPIRY_DELTA, userCustomTlvs = customRecords, routeParams = nodeParams.routerConf.pathFindingExperimentConf.getRandomConf().getDefaultRouteParams)
  ...
  test("forward payment with pre-defined route") { f =>
    ...
    val request = SendPaymentToRoute(finalAmount, finalAmount, invoice, ignoredFinalExpiryDelta, route, None, None, None, 0 msat, CltvExpiryDelta(0), Nil)
  ...
  test("forward single-part payment when multi-part deactivated", Tag("mpp_disabled")) { f =>
    ...
    val req = SendPaymentToNode(finalAmount, invoice, 1, /* ignored since the invoice provides it */ CltvExpiryDelta(12), routeParams = nodeParams.routerConf.pathFindingExperimentConf.getRandomConf().getDefaultRouteParams)
  ...
  test("forward multi-part payment with pre-defined route") { f =>
    ...
    val req = SendPaymentToRoute(finalAmount / 2, finalAmount, invoice, Channel.MIN_CLTV_EXPIRY_DELTA, route, None, None, None, 0 msat, CltvExpiryDelta(0), Nil)
    sender.send(initiator, req)
    val payment = sender.expectMsgType[SendPaymentToRouteResponse]
    payFsm.expectMsg(SendPaymentConfig(payment.paymentId, payment.parentId, None, paymentHash, finalAmount, c, Upstream.Local(payment.paymentId), Some(invoice), storeInDb = true, publishEvent = true, recordPathFindingMetrics = false, Nil))
  ...
  test("forward trampoline payment") { f =>
    ...
    val req = SendTrampolinePayment(finalAmount, invoice, b, Seq((trampolineFees, CltvExpiryDelta(12))), /* ignored since the invoice provides it */ CltvExpiryDelta(18), routeParams = nodeParams.routerConf.pathFindingExperimentConf.getRandomConf().getDefaultRouteParams)
    sender.send(initiator, req)
    val id = sender.expectMsgType[UUID]
  ...
  test("forward trampoline to legacy payment") { f =>
    import f._
    val invoice = Bolt11Invoice(Block.LivenetGenesisBlock.hash, Some(finalAmount), paymentHash, priv_c.privateKey, Left("Some eclair-mobile invoice"), CltvExpiryDelta(9))
    val trampolineFees = 21000 msat
  ...
  test("reject trampoline to legacy payment for 0-value invoice") { f =>
    ...
    val trampolineFees = 21000 msat
    val req = SendTrampolinePayment(finalAmount, invoice, b, Seq((trampolineFees, CltvExpiryDelta(12))), CltvExpiryDelta(9), routeParams = nodeParams.routerConf.pathFindingExperimentConf.getRandomConf().getDefaultRouteParams)
    sender.send(initiator, req)
    val id = sender.expectMsgType[UUID]
    val fail = sender.expectMsgType[PaymentFailed]
    assert(fail.id === id)
  ...
  test("reject trampoline payment with onion too big") { f =>
    ...
    val invoice = Bolt11Invoice(Block.LivenetGenesisBlock.hash, Some(finalAmount), paymentHash, priv_c.privateKey, Left("Much payment very metadata"), CltvExpiryDelta(9), features = featuresWithTrampoline, paymentMetadata = Some(paymentMetadata))
    val trampolineFees = 21000 msat
    val req = SendTrampolinePayment(finalAmount, invoice, b, Seq((trampolineFees, CltvExpiryDelta(12))), CltvExpiryDelta(18), routeParams = nodeParams.routerConf.pathFindingExperimentConf.getRandomConf().getDefaultRouteParams)
    sender.send(initiator, req)
    val id = sender.expectMsgType[UUID]
    val fail = sender.expectMsgType[PaymentFailed]
    assert(fail.id === id)
  ...
  test("retry trampoline payment") { f =>
    ...
    val req = SendTrampolinePayment(finalAmount, invoice, b, trampolineAttempts, CltvExpiryDelta(9), routeParams = nodeParams.routerConf.pathFindingExperimentConf.getRandomConf().getDefaultRouteParams)
    sender.send(initiator, req)
    val id = sender.expectMsgType[UUID]
  ...
  test("retry trampoline payment and fail") { f =>
    ...
    val failed = PaymentFailed(cfg.parentId, invoice.paymentHash, Seq(RemoteFailure(msg2.totalAmount, Nil, Sphinx.DecryptedFailurePacket(b, TemporaryNodeFailure))))
  ...
  test("forward trampoline payment with pre-defined route") { f =>
    ...
    val req = SendPaymentToRoute(finalAmount + trampolineFees, finalAmount, invoice, Channel.MIN_CLTV_EXPIRY_DELTA, route, None, None, None, trampolineFees, CltvExpiryDelta(144), Seq(b, c))
    sender.send(initiator, req)
    val payment = sender.expectMsgType[SendPaymentToRouteResponse]
