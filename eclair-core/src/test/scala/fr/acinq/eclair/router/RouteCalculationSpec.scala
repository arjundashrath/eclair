...
import fr.acinq.eclair.{BlockHeight, CltvExpiryDelta, Features, MilliSatoshi, MilliSatoshiLong, ShortChannelId, TimestampSecond, TimestampSecondLong, ToMilliSatoshiConversion, randomKey}
...
class RouteCalculationSpec extends AnyFunSuite with ParallelTestExecution {
  ...
  test("calculate simple route") {
    ...
    val Success(route :: Nil) = findRoute(g, BalancesEstimates.baseline(g, 1 day), a, e, DEFAULT_AMOUNT_MSAT, DEFAULT_MAX_FEE, numRoutes = 1, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
  ...
  test("check fee against max pct properly") {
    ...
    val Success(route :: Nil) = findRoute(g, BalancesEstimates.baseline(g, 1 day), a, e, DEFAULT_AMOUNT_MSAT, maxFee, numRoutes = 1, routeParams = routeParams, currentBlockHeight = BlockHeight(400000))
    assert(route2Ids(route) === 1 :: 2 :: 3 :: 4 :: Nil)
  }
  ...
  test("calculate the shortest path (correct fees)") {
    ...
    val Success(route :: Nil) = findRoute(graph, BalancesEstimates.baseline(graph, 1 day), a, d, amount, maxFee = 7 msat, numRoutes = 1, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
    val weightedPath = Graph.pathWeight(BalancesEstimates.baseline(graph, 1 day), a, route2Edges(route), amount, BlockHeight(0), Left(NO_WEIGHT_RATIOS), includeLocalChannelCost = false)
    ...
    for (g <- Seq(graph1, graph2, graph3)) {
      val Success(route1 :: Nil) = findRoute(g, BalancesEstimates.baseline(g, 1 day), a, d, amount, maxFee = 10 msat, numRoutes = 1, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
  ...
  test("calculate simple route (add and remove edges") {
    ...
    val Success(route1 :: Nil) = findRoute(g, BalancesEstimates.baseline(g, 1 day), a, e, DEFAULT_AMOUNT_MSAT, DEFAULT_MAX_FEE, numRoutes = 1, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
    assert(route2Ids(route1) === 1 :: 2 :: 3 :: 4 :: Nil)

    val graphWithRemovedEdge = g.removeEdge(ChannelDesc(ShortChannelId(3L), c, d))
    val route2 = findRoute(graphWithRemovedEdge, BalancesEstimates.baseline(g, 1 day), a, e, DEFAULT_AMOUNT_MSAT, DEFAULT_MAX_FEE, numRoutes = 1, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
  ...
  test("calculate the shortest path (hardcoded nodes)") {
    ...
    val Success(route :: Nil) = findRoute(graph, BalancesEstimates.baseline(graph, 1 day), f, i, DEFAULT_AMOUNT_MSAT, DEFAULT_MAX_FEE, numRoutes = 1, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
  ...
  test("calculate the shortest path (select direct channel)") {
    ...
    val Success(route1 :: route2 :: Nil) = findRoute(graph, BalancesEstimates.baseline(graph, 1 day), f, i, DEFAULT_AMOUNT_MSAT, DEFAULT_MAX_FEE, numRoutes = 2, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
  ...
  test("find a route using channels with htlMinimumMsat close to the payment amount") {
    ...
    val route = findRoute(graph, BalancesEstimates.baseline(graph, 1 day), f, i, DEFAULT_AMOUNT_MSAT, DEFAULT_MAX_FEE, numRoutes = 1, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
  ...
  test("no local channels") {
    ...
    val route = findRoute(g, BalancesEstimates.baseline(g, 1 day), a, e, DEFAULT_AMOUNT_MSAT, DEFAULT_MAX_FEE, numRoutes = 1, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
    assert(route === Failure(RouteNotFound))
  }
  ...
  test("route not found (source OR target node not connected)") {
    ...
    assert(findRoute(g, BalancesEstimates.baseline(g, 1 day), a, d, DEFAULT_AMOUNT_MSAT, DEFAULT_MAX_FEE, numRoutes = 1, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000)) === Failure(RouteNotFound))
    assert(findRoute(g, BalancesEstimates.baseline(g, 1 day), b, e, DEFAULT_AMOUNT_MSAT, DEFAULT_MAX_FEE, numRoutes = 1, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000)) === Failure(RouteNotFound))
  }
  ...
  test("route not found (amount too high OR too low)") {
    ...
    assert(findRoute(g, BalancesEstimates.baseline(g, 1 day), a, d, highAmount, DEFAULT_MAX_FEE, numRoutes = 1, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000)) === Failure(RouteNotFound))
    assert(findRoute(g1, BalancesEstimates.baseline(g1, 1 day), a, d, lowAmount, DEFAULT_MAX_FEE, numRoutes = 1, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000)) === Failure(RouteNotFound))
  }
  ...
  test("route not found (balance too low)") {
    ...
    assert(findRoute(g, BalancesEstimates.baseline(g, 1 day), a, d, 15000 msat, 100 msat, numRoutes = 1, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000)).isSuccess)
    ...
    Seq(g1, g2, g3).foreach(g => assert(findRoute(g, BalancesEstimates.baseline(g, 1 day), a, d, 15000 msat, 100 msat, numRoutes = 1, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000)) === Failure(RouteNotFound)))
  }
  ...
  test("route to self") {
    ...
    val route = findRoute(g, BalancesEstimates.baseline(g, 1 day), a, a, DEFAULT_AMOUNT_MSAT, DEFAULT_MAX_FEE, numRoutes = 1, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
  ...
  test("route to immediate neighbor") {
    ...
    val Success(route :: Nil) = findRoute(g, BalancesEstimates.baseline(g, 1 day), a, b, DEFAULT_AMOUNT_MSAT, DEFAULT_MAX_FEE, numRoutes = 1, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
  ...
  test("directed graph") {
    ...
    val route2 = findRoute(g, BalancesEstimates.baseline(g, 1 day), e, a, DEFAULT_AMOUNT_MSAT, DEFAULT_MAX_FEE, numRoutes = 1, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
    assert(route2 === Failure(RouteNotFound))
  }
  ...
  test("blacklist routes") {
    ...
    val route1 = findRoute(g, BalancesEstimates.baseline(g, 1 day), a, e, DEFAULT_AMOUNT_MSAT, DEFAULT_MAX_FEE, numRoutes = 1, ignoredEdges = Set(ChannelDesc(ShortChannelId(3L), c, d)), routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
    ...
    val Success(route2 :: Nil) = findRoute(g, BalancesEstimates.baseline(g, 1 day), a, e, DEFAULT_AMOUNT_MSAT, DEFAULT_MAX_FEE, numRoutes = 1, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
  ...
  test("route to a destination that is not in the graph (with assisted routes)") {
    ...
    val Success(route1 :: Nil) = findRoute(g, BalancesEstimates.baseline(g, 1 day), a, e, DEFAULT_AMOUNT_MSAT, DEFAULT_MAX_FEE, numRoutes = 1, extraEdges = extraGraphEdges, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
    assert(route2Ids(route1) === 1 :: 2 :: 3 :: 4 :: Nil)
  }
  ...
  test("route from a source that is not in the graph (with assisted routes)") {
    ...
    val route = findRoute(g, BalancesEstimates.baseline(g, 1 day), a, d, DEFAULT_AMOUNT_MSAT, DEFAULT_MAX_FEE, numRoutes = 1, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
    assert(route === Failure(RouteNotFound))

    // now we add the missing starting edge
    val extraGraphEdges = Set(makeEdge(1L, a, b, 5 msat, 5))
    val Success(route1 :: Nil) = findRoute(g, BalancesEstimates.baseline(g, 1 day), a, d, DEFAULT_AMOUNT_MSAT, DEFAULT_MAX_FEE, numRoutes = 1, extraEdges = extraGraphEdges, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
    assert(route2Ids(route1) === 1 :: 2 :: 3 :: Nil)
  }
  ...
  test("verify that extra hops takes precedence over known channels") {
    ...
    val Success(route2 :: Nil) = findRoute(g, BalancesEstimates.baseline(g, 1 day), a, e, DEFAULT_AMOUNT_MSAT, DEFAULT_MAX_FEE, numRoutes = 1, extraEdges = extraGraphEdges, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
    assert(route2Ids(route2) === 1 :: 2 :: 3 :: 4 :: Nil)
  ...
  test("limit routes to 20 hops") {
    ...
    assert(findRoute(g, BalancesEstimates.baseline(g, 1 day), nodes(0), nodes(18), DEFAULT_AMOUNT_MSAT, DEFAULT_MAX_FEE, numRoutes = 1, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000)).map(r => route2Ids(r.head)) === Success(0 until 18))
    assert(findRoute(g, BalancesEstimates.baseline(g, 1 day), nodes(0), nodes(19), DEFAULT_AMOUNT_MSAT, DEFAULT_MAX_FEE, numRoutes = 1, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000)).map(r => route2Ids(r.head)) === Success(0 until 19))
    assert(findRoute(g, BalancesEstimates.baseline(g, 1 day), nodes(0), nodes(20), DEFAULT_AMOUNT_MSAT, DEFAULT_MAX_FEE, numRoutes = 1, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000)).map(r => route2Ids(r.head)) === Success(0 until 20))
    assert(findRoute(g, BalancesEstimates.baseline(g, 1 day), nodes(0), nodes(21), DEFAULT_AMOUNT_MSAT, DEFAULT_MAX_FEE, numRoutes = 1, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000)) === Failure(RouteNotFound))
  }
  ...
  test("ignore cheaper route when it has more than 20 hops") {
    ...
    val Success(route :: Nil) = findRoute(g, BalancesEstimates.baseline(g, 1 day), nodes(0), nodes(49), DEFAULT_AMOUNT_MSAT, DEFAULT_MAX_FEE, numRoutes = 1, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
  ...
  test("ignore cheaper route when it has more than the requested CLTV") {
    ...
    val Success(route :: Nil) = findRoute(g, BalancesEstimates.baseline(g, 1 day), a, d, DEFAULT_AMOUNT_MSAT, DEFAULT_MAX_FEE, numRoutes = 1, routeParams = DEFAULT_ROUTE_PARAMS.modify(_.boundaries.maxCltv).setTo(CltvExpiryDelta(28)), currentBlockHeight = BlockHeight(400000))
    assert(route2Ids(route) === 4 :: 5 :: 6 :: Nil)
  }
  ...
  test("ignore cheaper route when it grows longer than the requested size") {
    ...
    val Success(route :: Nil) = findRoute(g, BalancesEstimates.baseline(g, 1 day), a, f, DEFAULT_AMOUNT_MSAT, DEFAULT_MAX_FEE, numRoutes = 1, routeParams = DEFAULT_ROUTE_PARAMS.modify(_.boundaries.maxRouteLength).setTo(3), currentBlockHeight = BlockHeight(400000))
  ...
  test("ensure the route calculation terminates correctly when selecting 0-fees edges") {
    ...
    val Success(route :: Nil) = findRoute(g, BalancesEstimates.baseline(g, 1 day), a, d, DEFAULT_AMOUNT_MSAT, DEFAULT_MAX_FEE, numRoutes = 1, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
  ...
  test("find the k-shortest paths in a graph, k=4") {
    ...
    val fourShortestPaths = Graph.yenKshortestPaths(g1, BalancesEstimates.baseline(g1, 1 day), d, f, DEFAULT_AMOUNT_MSAT, Set.empty, Set.empty, Set.empty, pathsToFind = 4, Left(NO_WEIGHT_RATIOS), BlockHeight(0), noopBoundaries, includeLocalChannelCost = false)
    ...
    val threeShortestPaths = Graph.yenKshortestPaths(g2, BalancesEstimates.baseline(g2, 1 day), d, f, DEFAULT_AMOUNT_MSAT, Set.empty, Set.empty, Set.empty, pathsToFind = 4, Left(NO_WEIGHT_RATIOS), BlockHeight(0), noopBoundaries, includeLocalChannelCost = false)
  ...
  test("find the k shortest path (wikipedia example)") {
    ...
    val twoShortestPaths = Graph.yenKshortestPaths(graph, BalancesEstimates.baseline(graph, 1 day), c, h, DEFAULT_AMOUNT_MSAT, Set.empty, Set.empty, Set.empty, pathsToFind = 2, Left(NO_WEIGHT_RATIOS), BlockHeight(0), noopBoundaries, includeLocalChannelCost = false)
  ...
  test("terminate looking for k-shortest path if there are no more alternative paths than k, must not consider routes going back on their steps") {
    ...
    val foundPaths = Graph.yenKshortestPaths(graph, BalancesEstimates.baseline(graph, 1 day), a, f, DEFAULT_AMOUNT_MSAT, Set.empty, Set.empty, Set.empty, pathsToFind = 3, Left(NO_WEIGHT_RATIOS), BlockHeight(0), noopBoundaries, includeLocalChannelCost = false)
  ...
  test("select a random route below the requested fee") {
    ...
    for (_ <- 0 to 10) {
      val Success(routes) = findRoute(g, BalancesEstimates.baseline(g, 1 day), a, d, DEFAULT_AMOUNT_MSAT, strictFee, numRoutes = 3, routeParams = strictFeeParams, currentBlockHeight = BlockHeight(400000))
      assert(routes.length === 2, routes)
      val weightedPath = Graph.pathWeight(BalancesEstimates.baseline(g, 1 day), a, route2Edges(routes.head), DEFAULT_AMOUNT_MSAT, BlockHeight(400000), Left(NO_WEIGHT_RATIOS), includeLocalChannelCost = false)
  ...
  test("use weight ratios when computing the edge weight") {
    ...
    val Success(routeFeeOptimized :: Nil) = findRoute(g, BalancesEstimates.baseline(g, 1 day), a, d, DEFAULT_AMOUNT_MSAT, DEFAULT_MAX_FEE, numRoutes = 1, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
    assert(route2Nodes(routeFeeOptimized) === (a, b) :: (b, c) :: (c, d) :: Nil)

    val Success(routeCltvOptimized :: Nil) = findRoute(g, BalancesEstimates.baseline(g, 1 day), a, d, DEFAULT_AMOUNT_MSAT, DEFAULT_MAX_FEE, numRoutes = 1, routeParams = DEFAULT_ROUTE_PARAMS.copy(heuristics = Left(WeightRatios(
      baseFactor = 0,
      cltvDeltaFactor = 1,
      ageFactor = 0,
      capacityFactor = 0,
    ...
    val Success(routeCapacityOptimized :: Nil) = findRoute(g, BalancesEstimates.baseline(g, 1 day), a, d, DEFAULT_AMOUNT_MSAT, DEFAULT_MAX_FEE, numRoutes = 1, routeParams = DEFAULT_ROUTE_PARAMS.copy(heuristics = Left(WeightRatios(
      baseFactor = 0,
      cltvDeltaFactor = 0,
      ageFactor = 0,
      capacityFactor = 1,
  ...
  test("prefer going through an older channel if fees and CLTV are the same") {
    ...
    val Success(routeScoreOptimized :: Nil) = findRoute(g, BalancesEstimates.baseline(g, 1 day), a, d, DEFAULT_AMOUNT_MSAT / 2, DEFAULT_MAX_FEE, numRoutes = 1, routeParams = DEFAULT_ROUTE_PARAMS.copy(heuristics = Left(WeightRatios(
      baseFactor = 0.01,
      ageFactor = 0.33,
  ...
  test("prefer a route with a smaller total CLTV if fees and score are the same") {
    ...
    val Success(routeScoreOptimized :: Nil) = findRoute(g, BalancesEstimates.baseline(g, 1 day), a, d, DEFAULT_AMOUNT_MSAT, DEFAULT_MAX_FEE, numRoutes = 1, routeParams = DEFAULT_ROUTE_PARAMS.copy(heuristics = Left(WeightRatios(
      baseFactor = 0.01,
      ageFactor = 0.33,
      cltvDeltaFactor = 0.33,
      capacityFactor = 0.33,
  ...
  test("cost function is monotonic") {
    ...
    val Success(route :: Nil) = findRoute(g, BalancesEstimates.baseline(g, 1 day), thisNode, targetNode, amount, DEFAULT_MAX_FEE, 1, Set.empty, Set.empty, Set.empty, params, currentBlockHeight = BlockHeight(567634)) // simulate mainnet block for heuristic
  ...
  test("calculate multipart route to neighbor (many channels, known balance)") {
    ...
    {
      val Success(routes) = findMultiPartRoute(g, BalancesEstimates.baseline(g, 1 day), a, b, amount, 1 msat, routeParams = routeParams, currentBlockHeight = BlockHeight(400000))
      assert(routes.length === 4, routes)
      assert(routes.forall(_.length == 1), routes)
      checkRouteAmounts(routes, amount, 0 msat)
    }
    {
      val Success(routes) = findMultiPartRoute(g, BalancesEstimates.baseline(g, 1 day), a, b, amount, 1 msat, routeParams = routeParams.copy(randomize = true), currentBlockHeight = BlockHeight(400000))
      assert(routes.length >= 4, routes)
      assert(routes.forall(_.length == 1), routes)
      checkRouteAmounts(routes, amount, 0 msat)
    }
    {
      ...
      val failure = findMultiPartRoute(g, BalancesEstimates.baseline(g, 1 day), a, b, amount, 1 msat, routeParams = routeParams.copy(mpp = MultiPartParams(16500 msat, 3)), currentBlockHeight = BlockHeight(400000))
  ...
  test("calculate multipart route to neighbor (single channel, known balance)") {
    ...
    val Success(routes) = findMultiPartRoute(g, BalancesEstimates.baseline(g, 1 day), a, b, amount, 1 msat, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
  ...
  test("calculate multipart route to neighbor (many channels, some empty)") {
    ...
    {
      val Success(routes) = findMultiPartRoute(g, BalancesEstimates.baseline(g, 1 day), a, b, amount, 1 msat, routeParams = DEFAULT_ROUTE_PARAMS.copy(randomize = true), currentBlockHeight = BlockHeight(400000))
      assert(routes.length >= 3, routes)
      assert(routes.forall(_.length == 1), routes)
      checkIgnoredChannels(routes, 2L)
      checkRouteAmounts(routes, amount, 0 msat)
  ...
  test("calculate multipart route to neighbor (ignored channels)") {
    ...
    val Success(routes) = findMultiPartRoute(g, BalancesEstimates.baseline(g, 1 day), a, b, amount, 1 msat, ignoredEdges = ignoredEdges, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
    assert(routes.forall(_.length == 1), routes)
  ...
  test("calculate multipart route to neighbor (pending htlcs ignored for local channels)") {
    ...
    val Success(routes) = findMultiPartRoute(g, BalancesEstimates.baseline(g, 1 day), a, b, amount, 1 msat, pendingHtlcs = pendingHtlcs, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
    assert(routes.forall(_.length == 1), routes)
    checkRouteAmounts(routes, amount, 0 msat)
  }
  ...
  test("calculate multipart route to neighbor (through remote channels)") {
    ...
    val amount = 30000 msat
    val maxFeeTooLow = findMultiPartRoute(g, BalancesEstimates.baseline(g, 1 day), a, b, amount, 1 msat, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
    assert(maxFeeTooLow === Failure(RouteNotFound))

    val Success(routes) = findMultiPartRoute(g, BalancesEstimates.baseline(g, 1 day), a, b, amount, 20 msat, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
  ...
  test("cannot find multipart route to neighbor (not enough balance)") {
    ...
    {
      val result = findMultiPartRoute(g, BalancesEstimates.baseline(g, 1 day), a, b, 40000 msat, 1 msat, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
      assert(result === Failure(RouteNotFound))
    }
    {
      val result = findMultiPartRoute(g, BalancesEstimates.baseline(g, 1 day), a, b, 40000 msat, 1 msat, routeParams = DEFAULT_ROUTE_PARAMS.copy(randomize = true), currentBlockHeight = BlockHeight(400000))
      assert(result === Failure(RouteNotFound))
    }
  }
  ...
  test("cannot find multipart route to neighbor (not enough capacity)") {
    ...
    val result = findMultiPartRoute(g, BalancesEstimates.baseline(g, 1 day), a, b, 5000000 msat, 1 msat, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
    assert(result === Failure(RouteNotFound))
  }
  ...
  test("cannot find multipart route to neighbor (restricted htlc_minimum_msat)") {
    ...
    {
      val result = findMultiPartRoute(g, BalancesEstimates.baseline(g, 1 day), a, b, 10000 msat, 1 msat, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
      assert(result === Failure(RouteNotFound))
    }
    {
      val result = findMultiPartRoute(g, BalancesEstimates.baseline(g, 1 day), a, b, 10000 msat, 1 msat, routeParams = DEFAULT_ROUTE_PARAMS.copy(randomize = true), currentBlockHeight = BlockHeight(400000))
      assert(result === Failure(RouteNotFound))
    }
  }
  ...
  test("calculate multipart route to remote node (many local channels)") {
    ...
    {
      val Success(routes) = findMultiPartRoute(g, BalancesEstimates.baseline(g, 1 day), a, e, amount, maxFee, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
    ...
    }
    {
      ...
      val Success(routes) = findMultiPartRoute(g1, BalancesEstimates.baseline(g1, 1 day), a, e, amount, maxFee, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
      checkRouteAmounts(routes, amount, maxFee)
      assert(routes2Ids(routes) === Set(Seq(1L, 2L, 3L), Seq(4L, 6L), Seq(5L, 6L)))
    }
    {
      // Randomize routes.
      val Success(routes) = findMultiPartRoute(g, BalancesEstimates.baseline(g, 1 day), a, e, amount, maxFee, routeParams = DEFAULT_ROUTE_PARAMS.copy(randomize = true), currentBlockHeight = BlockHeight(400000))
      checkRouteAmounts(routes, amount, maxFee)
    }
    {
      // Update balance A - B to be too low.
      val g1 = g.addEdge(edge_ab.copy(balance_opt = Some(2000 msat)))
      val failure = findMultiPartRoute(g1, BalancesEstimates.baseline(g1, 1 day), a, e, amount, maxFee, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
      assert(failure === Failure(RouteNotFound))
    }
    {
    ...
    }
    {
      ...
      val failure = findMultiPartRoute(g, BalancesEstimates.baseline(g, 1 day), a, e, amount, maxFeeTooLow, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
      assert(failure === Failure(RouteNotFound))
    }
  }
  ...
  test("calculate multipart route to remote node (tiny amount)") {
...

    {
      ...
      val Success(routes) = findMultiPartRoute(g, BalancesEstimates.baseline(g, 1 day), a, e, amount, maxFee, routeParams = routeParams, currentBlockHeight = BlockHeight(400000))
      checkRouteAmounts(routes, amount, maxFee)
    }
    {
      // But we don't want to split such tiny amounts.
      val (amount, maxFee) = (2000 msat, 150 msat)
      val failure = findMultiPartRoute(g, BalancesEstimates.baseline(g, 1 day), a, e, amount, maxFee, routeParams = routeParams, currentBlockHeight = BlockHeight(400000))
      assert(failure === Failure(RouteNotFound))
    }
  }
  ...
  test("calculate multipart route to remote node (single path)") {
    ...
    val Success(routes) = findMultiPartRoute(g, BalancesEstimates.baseline(g, 1 day), a, d, amount, maxFee, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
    checkRouteAmounts(routes, amount, maxFee)
  ...
  test("calculate multipart route to remote node (single local channel)") {
    ...
    {
      val Success(routes) = findMultiPartRoute(g, BalancesEstimates.baseline(g, 1 day), a, f, amount, maxFee, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
      checkRouteAmounts(routes, amount, maxFee)
      assert(routes2Ids(routes) === Set(Seq(1L, 2L, 3L, 5L), Seq(1L, 4L, 5L), Seq(1L, 6L, 7L)))
    }
    {
      // Randomize routes.
      val Success(routes) = findMultiPartRoute(g, BalancesEstimates.baseline(g, 1 day), a, f, amount, maxFee, routeParams = DEFAULT_ROUTE_PARAMS.copy(randomize = true), currentBlockHeight = BlockHeight(400000))
      checkRouteAmounts(routes, amount, maxFee)
    }
    {
      // Update A - B with unknown balance, capacity should be used instead.
      val g1 = g.addEdge(edge_ab.copy(capacity = 500 sat, balance_opt = None))
      val Success(routes) = findMultiPartRoute(g1, BalancesEstimates.baseline(g1, 1 day), a, f, amount, maxFee, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
      checkRouteAmounts(routes, amount, maxFee)
      assert(routes2Ids(routes) === Set(Seq(1L, 2L, 3L, 5L), Seq(1L, 4L, 5L), Seq(1L, 6L, 7L)))
    }
    {
      ...
      val failure = findMultiPartRoute(g1, BalancesEstimates.baseline(g1, 1 day), a, f, amount, maxFee, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
      assert(failure === Failure(RouteNotFound))
    }
    {
    ...
    }
    {
      // Try to find a route with a maxFee that's too low.
      val maxFeeTooLow = 100 msat
      val failure = findMultiPartRoute(g, BalancesEstimates.baseline(g, 1 day), a, f, amount, maxFeeTooLow, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
      assert(failure === Failure(RouteNotFound))
    }
  }
  ...
  test("calculate multipart route to remote node (ignore cheap routes with low capacity)") {
...

    {
      ...
      val Success(routes) = findMultiPartRoute(g, BalancesEstimates.baseline(g, 1 day), a, d, amount, maxFee, routeParams = routeParams, currentBlockHeight = BlockHeight(400000))
      checkRouteAmounts(routes, amount, maxFee)
    ...
    }
    {
      ...
      val routeParams = DEFAULT_ROUTE_PARAMS.copy(randomize = false, mpp = MultiPartParams(50_000 msat, 5))
      val failure = findMultiPartRoute(g, BalancesEstimates.baseline(g, 1 day), a, d, amount, maxFee, routeParams = routeParams, currentBlockHeight = BlockHeight(400000))
      assert(failure === Failure(RouteNotFound))
    }
    {
  ...
  test("calculate multipart route to remote node (ignored channels and nodes)") {
    ...
    val Success(routes) = findMultiPartRoute(g, BalancesEstimates.baseline(g, 1 day), a, f, amount, maxFee, ignoredEdges = ignoredChannels, ignoredVertices = ignoredNodes, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
    checkRouteAmounts(routes, amount, maxFee)
  ...
  test("calculate multipart route to remote node (complex graph)") {
...

    {
      ...
      val Success(routes) = findMultiPartRoute(g, BalancesEstimates.baseline(g, 1 day), d, f, amount, maxFee, routeParams = routeParams, currentBlockHeight = BlockHeight(400000))
      checkRouteAmounts(routes, amount, maxFee)
    }
    {
    ...
    }
    {
      ...
      val failure = findMultiPartRoute(g, BalancesEstimates.baseline(g, 1 day), d, f, amount, maxFee, routeParams = routeParams, currentBlockHeight = BlockHeight(400000))
      assert(failure === Failure(RouteNotFound))
    }
    {
    ...
    {
      val (amount, maxFee) = (40000 msat, 100 msat)
      val Success(routes) = findMultiPartRoute(g, BalancesEstimates.baseline(g, 1 day), d, f, amount, maxFee, routeParams = routeParams.copy(randomize = true), currentBlockHeight = BlockHeight(400000))
      checkRouteAmounts(routes, amount, maxFee)
    }
    {
  ...
  test("calculate multipart route to remote node (with extra edges)") {
    ...
    val Success(routes1) = findMultiPartRoute(g, BalancesEstimates.baseline(g, 1 day), a, e, amount, maxFeeE, extraEdges = extraEdges, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
    checkRouteAmounts(routes1, amount, maxFeeE)
    assert(routes1.length >= 3, routes1)
    assert(routes1.forall(_.amount <= 4000.msat), routes1)

    val Success(routes2) = findMultiPartRoute(g, BalancesEstimates.baseline(g, 1 day), a, f, amount, maxFeeF, extraEdges = extraEdges, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
    ...
    val failure = findMultiPartRoute(g, BalancesEstimates.baseline(g, 1 day), a, f, amount, maxFeeTooLow, extraEdges = extraEdges, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
    assert(failure === Failure(RouteNotFound))
  }
  ...
  test("calculate multipart route to remote node (pending htlcs)") {
    ...
    val Success(routes) = findMultiPartRoute(g, BalancesEstimates.baseline(g, 1 day), a, e, amount, maxFee, pendingHtlcs = pendingHtlcs, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
    assert(routes.forall(_.length == 2), routes)
    checkRouteAmounts(routes, amount, maxFee)
    checkIgnoredChannels(routes, 1L, 2L)
  }
  ...
  test("calculate multipart route for full amount or fail", Tag("fuzzy")) {
    ...
    for (_ <- 1 to 100) {
      ...
      findMultiPartRoute(g, BalancesEstimates.baseline(g, 1 day), d, f, amount, maxFee, routeParams = DEFAULT_ROUTE_PARAMS.copy(randomize = true), currentBlockHeight = BlockHeight(400000)) match {
  ...
  test("loop trap") {
    ...
    val Success(routes) = findRoute(g, BalancesEstimates.baseline(g, 1 day), a, e, DEFAULT_AMOUNT_MSAT, DEFAULT_MAX_FEE, numRoutes = 3, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
  ...
  test("reversed loop trap") {
    ...
    val Success(routes) = findRoute(g, BalancesEstimates.baseline(g, 1 day), e, a, DEFAULT_AMOUNT_MSAT, DEFAULT_MAX_FEE, numRoutes = 3, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
    assert(routes.length == 2)
    val route1 :: route2 :: Nil = routes
  ...
  test("k-shortest paths must be distinct") {
    ...
    val Success(routes) = findRoute(g, BalancesEstimates.baseline(g, 1 day), a, b, DEFAULT_AMOUNT_MSAT, 100000000 msat, numRoutes = 10, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
  ...
  test("can't relay if fee is not sufficient") {
    ...
    assert(findRoute(g, BalancesEstimates.baseline(g, 1 day), a, b, 10000000 msat, 10000 msat, numRoutes = 3, routeParams = DEFAULT_ROUTE_PARAMS.copy(includeLocalChannelCost = true), currentBlockHeight = BlockHeight(400000)) === Failure(RouteNotFound))
    assert(findRoute(g, BalancesEstimates.baseline(g, 1 day), a, b, 10000000 msat, 100000 msat, numRoutes = 3, routeParams = DEFAULT_ROUTE_PARAMS.copy(includeLocalChannelCost = true), currentBlockHeight = BlockHeight(400000)).isSuccess)
  }
  ...
  test("penalty per hop") {
    ...
    { // No hop cost
      val Success(routes) = findRoute(g, BalancesEstimates.baseline(g, 1 day), start, b, DEFAULT_AMOUNT_MSAT, 100000000 msat, numRoutes = 1, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
      assert(routes.distinct.length == 1)
      val route :: Nil = routes
      assert(route2Ids(route) === 0 :: 2 :: 5 :: 6 :: 7 :: 4 :: Nil)
    }
    { // small base hop cost
      val Success(routes) = findRoute(g, BalancesEstimates.baseline(g, 1 day), start, b, DEFAULT_AMOUNT_MSAT, 100000000 msat, numRoutes = 1, routeParams = DEFAULT_ROUTE_PARAMS.copy(heuristics = Left(WeightRatios(1, 0, 0, 0, RelayFees(100 msat, 0)))), currentBlockHeight = BlockHeight(400000))
      assert(routes.distinct.length == 1)
      val route :: Nil = routes
      assert(route2Ids(route) === 0 :: 2 :: 3 :: 4 :: Nil)
    }
    { // large proportional hop cost
      val Success(routes) = findRoute(g, BalancesEstimates.baseline(g, 1 day), start, b, DEFAULT_AMOUNT_MSAT, 100000000 msat, numRoutes = 1, routeParams = DEFAULT_ROUTE_PARAMS.copy(heuristics = Left(WeightRatios(1, 0, 0, 0, RelayFees(0 msat, 200)))), currentBlockHeight = BlockHeight(400000))
      assert(routes.distinct.length == 1)
      val route :: Nil = routes
  ...
  test("we know path has high liquidity") {
    //  S ---> A ---> B
    //         |      ^
    //         v      |
    //         C ---> D
    val start = randomKey().publicKey
    val abEdge = makeEdge(1L, a, b, 1000 msat, 1000, capacity = (DEFAULT_AMOUNT_MSAT * 1.2).truncateToSatoshi)
    val g = DirectedGraph(List(
      makeEdge(0L, start, a, 0 msat, 0),
      abEdge,
      makeEdge(2L, a, c, 400 msat, 500, capacity = (DEFAULT_AMOUNT_MSAT * 3).truncateToSatoshi),
    ...
    val balances = BalancesEstimates.baseline(g, 1 day).channelCouldSend(ChannelHop(a, b, abEdge.update), DEFAULT_AMOUNT_MSAT * 0.9)
  ...
  test("we know path has low liquidity") {
    //  S ---> A ---> B
    //         |      ^
    //         v      |
    //         C ---> D
    val start = randomKey().publicKey
    val cdEdge = makeEdge(3L, c, d, 400 msat, 500, capacity = (DEFAULT_AMOUNT_MSAT * 3).truncateToSatoshi)
    val g = DirectedGraph(List(
      makeEdge(0L, start, a, 0 msat, 0),
      makeEdge(1L, a, b, 1000 msat, 1000, capacity = (DEFAULT_AMOUNT_MSAT * 1.2).truncateToSatoshi),
      makeEdge(2L, a, c, 400 msat, 500, capacity = (DEFAULT_AMOUNT_MSAT * 3).truncateToSatoshi),
    ...
    val balances = BalancesEstimates.baseline(g, 1 day).channelCouldNotSend(ChannelHop(c, d, cdEdge.update), DEFAULT_AMOUNT_MSAT * 1.1)
